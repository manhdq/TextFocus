...
class TextInstance(object):
    ...
    def find_bottom_and_sideline(self):
        ...
        self.e1, self.e2 = find_long_edges(
            self.points, self.bottoms
        )  # Find two long edge sequence
    ...
    def get_control_points(self, size=None):
        ...
        control_points = np.concatenate(
            [np.array(top), np.array(bot[::-1])], axis=0
        ).astype(np.float32)
...
class TextDataset(object):
    ...
    def generate_proposal_point(
        text_mask, num_points, approx_factor, jitter=0.0, distance=10.0
    ):
    ...
    def make_text_region(self, img, polygons):
        ...
        proposal_points = np.zeros(
            (cfg.max_annotation, cfg.num_points, 2), dtype=np.float
        )
