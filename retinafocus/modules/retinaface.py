...
class ConfHead(nn.Module):
    def __init__(self, inchannels=512, num_anchors=3):
        ...
        ##TODO: convert self.num_classes to local params
...
class RetinaFace(nn.Module):
    def __init__(self, cfg=None, phase='train'):
        ...
        self.ConfHead = self._make_conf_head(fpn_num=3,
    ...
    def _make_conf_head(self, fpn_num=3, inchannels=64, num_anchors=2):
        confhead = nn.ModuleList()
        for i in range(fpn_num):
            confhead.append(ConfHead(inchannels, num_anchors))
        return confhead
    ...
    def forward(self, inputs):
        ...
        classifications = torch.cat([self.ClassHead[i](feature)
                                     for i, feature in enumerate(features)],
                                    dim=1)
        confidences = torch.cat([self.ConfHead[i](feature)
                                 for i, feature in enumerate(features)],
                                dim=1)
