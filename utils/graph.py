...
def graph_propagation_soft(edges, score, max_sz, step=0.1, **kwargs):
    ...
    fusion_vertex_idx = np.setdiff1d(
        np.arange(nodes.shape[0]), first_vertex_idx, assume_unique=True
    )
    # Iteration
    components = comps[:]
    while remain:
...
def diffusion(vertex, label, score_dict, max_depth=5, weight_decay=0.6, normalize=True):
    ...
    for root in vertex:
        if prog % prog_step == 0:
            print(
                "progress: {} / {}, elapsed time: {}".format(
                    prog, len(vertex), time.time() - start
                )
            )
        prog += 1
        # queue = {[root, 0, 1.0]}
        ...
        while queue:
            ...
            for n in neighbors:
                if n.name not in visited:
                    sub_value = (
                        score_dict[tuple(sorted([curr.node.name, n.name]))]
                        * weight_decay
                        * curr.value
                    )
