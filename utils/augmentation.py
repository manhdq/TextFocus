...
class Rotate(object):
    ...
    def __call__(self, img, polygons=None):
        if np.random.randint(2):
            return img, polygons
        angle = np.random.normal(loc=0.0, scale=0.5) * self.up
...
class RotatePadding(object):
    ...
    @staticmethod
    def rotate(center, pt, theta, movSize=[0, 0], scale=1):
        (xr, yr) = center
        yr = -yr
        x, y = pt[:, 0], pt[:, 1]
        y = -y
    ...
    def __call__(self, image, polygons=None, scale=1.0):
        ...
        if self.colors:
            ...
            mask = cv2.warpAffine(
                mask, M, (newSize[0], newSize[1]), borderValue=[1, 1, 1]
            )
            image = cv2.warpAffine(
                image, M, (newSize[0], newSize[1]), borderValue=[0, 0, 0]
            )
            image = image + img_cut * mask
        else:
            color = [0, 0, 0]
            image = cv2.warpAffine(
                image, M, (newSize[0], newSize[1]), borderValue=color
            )
...
class ResizeSquare(object):
    ...
    def __call__(self, image, polygons=None):
        ...
        if img_size_min < self.size[0]:
            im_scale = float(self.size[0]) / float(
                img_size_min
            )  # expand min to size[0]
            if (
                np.ceil(im_scale * img_size_max) > self.size[1]
            ):  # expand max can't > size[1]
...
class RandomResizePadding(object):
    def __init__(
        self,
        size=512,
        random_scale=np.array([0.75, 1.0, 1.25, 1.5, 2.0]),
        stride=32,
        ratio=0.6667,
    ):
...
class Augmentation(object):
    def __init__(self, size, mean, std):
        ...
        self.augmentation = Compose(
            [
                RandomCropFlip(),
                RandomResizeScale(size=self.size, ratio=(3.0 / 8, 5.0 / 2)),
                RandomResizedCrop(),
                RotatePadding(
                    up=60, colors=True
                ),  # Pretrain on Syn is "up=30", else is "up=60"
