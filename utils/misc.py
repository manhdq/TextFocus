...
def point_dist_to_line(line, p3):
    ...
    if l2(d) > 0:
        distance = abs(
            d[1] * p3[0] - d[0] * p3[1] + p2[0] * p1[1] - p2[1] * p1[0]
        ) / l2(d)
    else:
    ...
    if len(pts) > 4:
        ...
        for i in range(1, len(pts) + 1):
            ...
            if cos(v_prev, v_next) < -0.875:
                candidate.append(
                    (i % len(pts), (i + 1) % len(pts), norm2(e[i] - e[i + 1]))
                )

        if (
            len(candidate) != 2
            or candidate[0][0] == candidate[1][1]
            or candidate[0][1] == candidate[1][0]
        ):
            ...
            if len(candidate) > 2:
                ...
                if l1 > 2 * l2 or l2 > 2 * l1 or len1 == 0 or len2 == 0:
                    ...
                    bottom_idx = np.argsort(
                        [dist for s1, e1, s2, e2, dist in dist_list]
                    )[-1]
                ...
                for i in range(len(pts)):
                    for j in range(len(pts)):
                        ...
                        dist = norm2(mid1 - mid2)
                        dist_list.append((s1, e1, s2, e2, dist))
                bottom_idx = np.argsort([dist for s1, e1, s2, e2, dist in dist_list])[
                    -2:
                ]
