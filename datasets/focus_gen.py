...
class FocusGenerator():
    ...
    def __init__(self,
                 config):
        self.dont_care_low = config['dont_care_low']
        self.dont_care_high = config['dont_care_high']
        self.small_threshold = config['small_threshold']
        ##TODO: Dynamic this
        kernel_size = 3
    ...
    def get_mask_flag_from_landmarks(self, lms):
        lms = lms.copy() / self.stride  ##TODO: careful this
    ...
    ##TODO: Change name
    def calculate_mask_by_landmarks_group(self, lms_group, text_tags, c_mask):
        lms_dont_care_list = []
        lms_care_list = []
        for lms, tag in zip(lms_group, text_tags):
            if not tag:
        ...
        cv2.fillPoly(dont_care_mask, [(lms.copy() / self.stride).astype(int) for lms in lms_dont_care_list], 1)
        # dont_care_mask = cv2.dilate(dont_care_mask, self.dilation_kernel)
        cv2.fillPoly(care_mask, [(lms.copy() / self.stride).astype(int) for lms in lms_care_list], 1)
