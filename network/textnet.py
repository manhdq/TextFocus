...
import cv2
from utils.misc import get_sample_point, fill_hole
from network.layers.gcn_utils import (
    get_node_feature,
    get_adj_mat,
    get_adj_ind,
    coord_embedding,
...
class Evolution(nn.Module):
    ...
    def get_boundary_proposal(input=None, seg_preds=None, switch="gt"):
        if switch == "gt":
            ...
            inds = (
                torch.from_numpy(np.array(inds)).permute(1, 0).to(input["img"].device)
            )
    ...
    def get_boundary_proposal_eval(self, input=None, seg_preds=None):
        ...
        if len(inds) > 0:
            inds = (
                ...
                .permute(1, 0)
                .to(input["img"].device, non_blocking=True)
            )
            init_polys = (
                torch.from_numpy(np.array(init_polys))
                .to(input["img"].device, non_blocking=True)
                .float()
            )
        else:
            ...
            inds = torch.from_numpy(np.array(inds)).to(
                input["img"].device, non_blocking=True
            )
    ...
    def evolve_poly(self, snake, cnn_feature, i_it_poly, ind):
        ...
        i_poly = i_it_poly + torch.clamp(
            snake(node_feats, self.adj).permute(0, 2, 1), -self.clip_dis, self.clip_dis
        )
        if self.is_training:
...
class TextNet(nn.Module):
    ...
    def forward(self, input_dict, test_speed=False):
        ...
        if (
            ...
            image = torch.zeros(
                (b, c, cfg.test_size[1], cfg.test_size[1]), dtype=torch.float32
            ).to(cfg.device)
        ...
        fy_preds = torch.cat(
            [torch.sigmoid(preds[:, 0:2, :, :]), preds[:, 2:4, :, :]], dim=1
        )
