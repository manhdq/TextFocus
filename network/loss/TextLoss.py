...
class TextLoss(nn.Module):
        ...
        for i in range(batch_size):
            ...
            if positive_pixel != 0:
                ...
                if len(pre_loss[i][(loss_label[i] < eps)]) < 3 * positive_pixel:
                    ...
                    nega_loss = torch.mean(
                        torch.topk(
                            pre_loss[i][(loss_label[i] < eps)], 3 * positive_pixel
                        )[0]
                    )
    ...
    def cls_ohem(self, predict, target, train_mask, negative_ratio=3.0):
        ...
        if n_pos.item() > 0:
            ...
            n_neg = min(
                int(neg.float().sum().item()), int(negative_ratio * n_pos.float())
            )
        else:
    ...
    def loss_calc_flux(pred_flux, gt_flux, weight_matrix, mask, train_mask):
        ...
        norm_loss = (
            weight_matrix * torch.mean((pred_flux - gt_flux) ** 2, dim=1) * train_mask
        )
        ...
        pred_flux = (
            0.999999 * pred_flux / (pred_flux.norm(p=2, dim=1).unsqueeze(1) + 1e-3)
        )
    ...
    def get_poly_energy(energy_field, img_poly, ind, h, w):
        ...
        gcn_feature = torch.zeros(
            [img_poly.size(0), energy_field.size(1), img_poly.size(1)]
        ).to(img_poly.device)
        for i in range(batch_size):
            poly = img_poly[ind == i].unsqueeze(0)
            gcn_feature[ind == i] = torch.nn.functional.grid_sample(
                energy_field[i : i + 1], poly
            )[0].permute(1, 0, 2)
        return gcn_feature
    ...
    def forward(self, input_dict, output_dict, eps=None):
        ...
        if cfg.scale > 1:
            ...
            distance_field = F.interpolate(
                distance_field.unsqueeze(1), scale_factor=1 / cfg.scale, mode="bilinear"
            ).squeeze()
            direction_field = F.interpolate(
                direction_field, scale_factor=1 / cfg.scale, mode="bilinear"
            )
            weight_matrix = F.interpolate(
                weight_matrix.unsqueeze(1), scale_factor=1 / cfg.scale, mode="bilinear"
            ).squeeze()
        ...
        if eps is None:
            ...
            beta = 3.0
            theta = 0.5
            gama = 0.1 * torch.sigmoid(
                torch.tensor((eps - cfg.max_epoch) / cfg.max_epoch)
            )
        loss = (
            alpha * cls_loss
            + beta * dis_loss
