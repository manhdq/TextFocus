...
from utils.visualize import visualize_detection, visualize_gt
from utils.misc import to_device, mkdirs, rescale_result
...
def inference(model, test_loader, output_dir):
    ...
    for i, (image, meta) in enumerate(test_loader):
        input_dict = dict()
        # Test mode can only run with batch_size == 1
        idx = 0
        ...
        if cfg.viz:
            ...
            for annot, n_annot in zip(
                meta["annotation"][idx], meta["n_annotation"][idx]
            ):
                if n_annot.item() > 0:
                    gt_contour.append(annot[:n_annot].int().cpu().numpy())

            gt_vis = visualize_gt(img_show, gt_contour, label_tag)
            show_boundary, heat_map = visualize_detection(
                img_show, output_dict, meta=meta
            )
...
def main(vis_dir_path):
    ...
    test_loader = data.DataLoader(
        testset, batch_size=1, shuffle=False, num_workers=cfg.num_workers
    )
